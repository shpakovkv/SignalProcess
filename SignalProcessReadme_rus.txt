...
def compare_2_files(first_file_name, second_file_name, lines=10):
    ...
    # Возвращает: True в случае совпадения, False при первом обнаруженном несоответствии
...
def peak_finder(x,                  # Одномерный массив времени (типа numpy.ndarray)
                y,                  # Одномерный массив значений (типа numpy.ndarray)
                level,              # Максимумы ниже этого уровня не участвуют в рассмотрении
                diffwindow,         # Минимальное расстояние между двумя пиками (размерность такая же как у элементов массива "х")
                tnoise=None,        # Проводится проверка на переполюсовку в пределах этого интервала времени относительно найденного пика
                                    #   так паразитные пики порожденные наводкой исключаются из рассмотрения
                is_negative=True,   # Указать полярность сигнала: Положительная = False, отрицательная = True (по умолчанию)
                graph=False):       # Отобразить найденные пики на графике или нет?
    # Находит максимумы кривой "y"
    # Возвращает двумерный массив (список списков) с "х" и "у" координатами пиков = [[x1, x2, x3,...], [y1, y2, y3,...]]
